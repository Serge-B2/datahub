...
class SupersetSource(StatefulIngestionSourceBase):
    ...
    def emit_chart_mces(self) -> Iterable[MetadataWorkUnit]:
        ...
        for field in columns:
            ...
            native_data_type = (
                field.get("type", "UNKNOWN") if field.get("type") else "UNKNOWN"
            )
            ...
            if calculated:
                global_tags = GlobalTagsClass(
                    tags=[TagAssociationClass(f"urn:li:tag:Calculated")]
                )
    ...
    def construct_dataset_snapshot(self, dataset_data: dict) -> DatasetSnapshot:
        ...
        dataset_snapshot = DatasetSnapshot(
            urn=dataset_urn, aspects=[dataset_properties, dataset_status]
        )
        ...
        owner = self._get_ownership(usernames) if usernames else None
